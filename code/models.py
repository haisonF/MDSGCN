...
def constructNet(miRNA_disease=None, miRNA_similarity=None, disease_similarity=None):
    if miRNA_disease is None:
        # Special purpose: unless you know what you are doing, don't let this parameter dbe None.
    ...
    #print("1111",np.vstack((m1,m2)).shape)
    return np.vstack((m1, m2))

def getNeighborsFromM(fr0, fr1, fr2, Mtx) -> set:   #set([ind[0]]),None,None, Mtx 
    pure_up = set()    
    pure_dn = set()
    mixed = set()
    # direct
...
def links2subgraphs(Mtx, Mtxori, train_pos,train_neg, test_pos,test_neg, h=1, max_nodes_per_hop=None, featM=None,featD=None) -> list:
    ...
    def helper(M, links, g_label):
        ...
        start = time.time()
        #(ind, Mtx:np.matrix, h=1, max_nodes_per_hop=None, featM=None, featD=None
        results = [subgraph_extraction_labeling(*((i, j), M, 1, max_nodes_per_hop, featM, featD)) for i, j in zip(links[0], links[1])]
...
from torch.autograd import Variable
from torch.nn.parameter import Parameter
from torch_geometric.nn import DenseGCNConv, DenseGraphConv,RGCNConv
...
class GCN(nn.Module):   #top feate=5
    def __init__(self, num_node_feats, hidden_dim=[64, 32], k=25, conv1d_kws=[0, 5]):
        ...
        self.locations = None
        self.__hid_type = 'e'  #! 't'=just topo  's'=just bio  'c'=combine topo and bio  c
        weights_init(self)
    ...
    def forward(self, graph_list):
        ...
        node_feat = one_hot(node_feat, max_n) 
    ...
    def pooling(self, node_feat, concat_feat, n2n_sp, graph_sizes, node_degs):
        ...
        for i in range(len(graph_sizes)):  
            ...
            if k < self.k:
                to_pad = torch.zeros(self.k-k, self.total_hidden_dim) 
...
class Mymodel(nn.Module):
    ...
    def forward(self, batch_input):
        embedding = self.gcn(batch_input)  

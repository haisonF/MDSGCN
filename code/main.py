...
def getMetrics(preds, targets,all_scores1):
    preds=np.array(preds)
    targets = np.array(targets)
    all_targets = np.array(targets)

    fpr1,tpr1,_=metrics.roc_curve(targets,all_scores1)
    auc1=metrics.auc(fpr1,tpr1)
    acc=metrics.accuracy_score(targets, preds)
 
    pr, re, _ = metrics.precision_recall_curve(targets,all_scores1)
    ...
    return [aupr, auc1,acc,precision, recall, f1]



def getMacroMetrics(predict_score, real_score,all_scores1):
    """
...
def train(model, g_list, sample_idxes, optimizer=None, criterion=None, bsize=50, epoch=-1, type='train',cv=-1):
    ...
    return getMetrics(np.array(all_scores), all_targets,np.array(all_scores1))
...
def mask_array(md: np.array, mask_rate=0.1):
    shape = md.shape
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input',default="../data/adj.csv",required=False)  #m_d
    parser.add_argument('-d', '--drsim-file', default="../data/dr_sim.csv", required=False)
    parser.add_argument('-m', '--musim-file', default="../data/mut_sim.csv", required=False)
    parser.add_argument('-b', '--bio-dim', default=128) 
    parser.add_argument('-o', '--output', default='result.txt')
    parser.add_argument('--hop', default=1, type=int)  #hop=1
    ...
    parser.add_argument('-t', '--trunc', default=75,type=int)  
    parser.add_argument('-mask', '--mask', default=0.1,type=float)
    ...
    if args.drsim_file is None:
        mm = np.eye(md.shape[0])
    elif args.drsim_file[-4:] == '.csv':
        mm = np.loadtxt(args.drsim_file, delimiter=',')
        #mm=pd.read_csv(args.misim_file,header=None)
        #mm=round(mm,2)
    elif args.drsim_file[-4:] == '.npz':
        mm = np.load(args.misim_file)['data']
    else:
        raise 'Error file extension name!'

    if args.musim_file is None: 
        dd = np.eye(md.shape[1])
    elif args.musim_file[-4:] == '.csv':
        dd = np.loadtxt(args.musim_file, delimiter=',')
        #dd=pd.read_csv(args.disim_file,header=None)
        #dd=round(dd,2)
    elif args.disim_file[-4:] == '.npz':
        dd = np.load(args.musim_file)['data']
    else:
        raise 'Error file extension name!'
    ...
    if load_split_data:
        data = np.load("../split_data_0.{}.npz".format(args.mask), allow_pickle=True)  
        kfolds = data['kfolds']
        kf_pos = data['kf_pos']
        kf_neg = data['kf_neg']
    else:
